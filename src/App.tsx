...
const App = () => {
  ...
  const canvasClick = (event: React.MouseEvent<HTMLCanvasElement, MouseEvent>) => {
    ...
    <>
      ...
      <div className="battleship-container">
            ...
            <tbody>
              {
                Object.entries(shipTypes).map(([shipType, details], index) => (
                  <tr key={index}>
                    <td><img src={`/images/${details.img}`} width={200}/></td>
                    <td className="score-hit">
                      {
                        Array(details.size).fill(0).map((_, index) => (
                          score[shipType] < index + 1 ?
                          <img src="/images/Miss small.png" key={index} width={20} className="dots" /> :
                          <img src="/images/Hit small.png" key={index} width={20} className="dots" />
                        ))
                      }
                    </td>
                  </tr>
        ...
        <div className="gameboard">
            <canvas ref={canvasRef} width={1000} height={1000} onClick={canvasClick}/>
        </div>
      </div>
    </>
  );
